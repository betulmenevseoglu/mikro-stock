...
export const metadata = { title: "Müşteriler — Mikro Stok Takip" };
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">{musteriler.length} kayıt</p>
        </div>
        <Link href="/musteriler/yeni" className="btn-primary">
      ...
      {musteriler.length === 0 ? (
        <div className="card flex flex-col items-center gap-3 px-6 py-16 text-center">
          ...
          <p className="text-sm text-slate-500">Henüz müşteri eklenmemiş.</p>
          <Link href="/musteriler/yeni" className="btn-secondary">
            <Plus className="h-4 w-4" />
            İlk müşteriyi ekle
          </Link>
        </div>
          ...
          <table className="table">
            <thead>
              <tr>
                ...
                <th>Yetkili</th>
                <th>İletişim</th>
                <th className="text-right">Özel Ürün</th>
            ...
            <tbody>
              {musteriler.map((m) => (
                <tr key={m.id}>
                  <td>
                    <Link href={`/musteriler/${m.id}`} className="block">
